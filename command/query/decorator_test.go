...
func TestDecorator(t *testing.T) {
	Convey("NewDecorator", t, func() {
		So(func() { NewDecorator("", Args{}) }, ShouldNotPanic)
	})
	...
	Convey("Apply", t, func() {
		...
		Convey("When the statement doesn't include the placeholder of decorator", func() {
			...
			d := NewDecorator(statement, Args{startDate: "2015-07-08 17:00:00"})
			actual, err := d.Apply()
		...
		Convey("When the statement includes the placeholder of decorator", func() {
			...
			Convey("The argument includes the startDate", func() {
				...
				Convey("The argument of startDate is formatted date string", func() {
					...
					Convey("The argument of tz is 0, which means UTC", func() {
						d := NewDecorator(statement, Args{startDate: startDate, buffer: 1})
						actual, err := d.Apply()
					...
					Convey("The argument of tz is -9, which means JST", func() {
						d := NewDecorator(statement, Args{startDate: startDate, tz: -9, buffer: 1})
						actual, err := d.Apply()
				...
				Convey("The argument of startDate is not formatted date string", func() {
					...
					d := NewDecorator(statement, Args{startDate: startDate})
					actual, err := d.Apply()
			...
			Convey("The argument includes the startDate and endDate", func() {
				...
				Convey("The argument of startDate and endDate are formatted date string", func() {
					...
					Convey("The argument of tz is 0, which means UTC", func() {
						d := NewDecorator(statement, Args{startDate: startDate, endDate: endDate, buffer: 1})
						actual, err := d.Apply()
					...
					Convey("The argument of tz is -9, which means JST", func() {
						d := NewDecorator(statement, Args{startDate: startDate, endDate: endDate, tz: -9, buffer: 1})
						actual, err := d.Apply()
				...
				Convey("The argument of endDate is not formatted date string", func() {
					...
					d := NewDecorator(statement, Args{startDate: startDate, endDate: endDate})
					actual, err := d.Apply()
			...
			Convey("The argument includes the beforeHour", func() {
				...
				Convey("The argument of beforeHour is greater than 0", func() {
					d := NewDecorator(statement, Args{beforeHour: 3.0, buffer: 1})
					actual, err := d.Apply()
				...
				Convey("The argument of beforeHour is 0", func() {
					d := NewDecorator(statement, Args{beforeHour: 1.0, buffer: 1})
					actual, err := d.Apply()
		...
		Convey("When the statement includes the placeholders of decorator and customFunc", func() {
			...
			d := NewDecorator(statement, Args{beforeHour: 3.0, tz: -9.0, buffer: 1})
			actual, err := d.Apply()
	...
	Convey("Revert", t, func() {
		...
		d := NewDecorator(statement, Args{tz: -9})
